...
for 2 .. * -> $number {
  ...
  LAST {
    say "Max Number tried ", $number if $verbose
  }
}
...
sub happy($n, $seq = ""){
  say :$seq ~ " %happiness{$n}<zhappy>" if $seq && %happiness{$n} && $verbose;
  my sub is-happy {
    my $next = $normalize-num($n.base($base).comb
      .map( *.parse-base($base) ** $pow).sum);
    %happiness{$n} = %(iter=>1);
    return %happiness{$n} = %(|%($_),:$next,iter=>$_<iter>+1)
      with happy($next, ($seq.Bool ?? $seq !! $n) ~ " => $next");
  }
